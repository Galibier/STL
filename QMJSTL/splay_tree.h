...
namespace qmj {
    ...
    struct splay_tree_node : public splay_tree_base_node<value_type> {
        ...
        splay_tree_node(const value_type &val, link_type left = nullptr, link_type right = nullptr)
                : value(std::move(val)), base_type(left, right) {}

        splay_tree_node(value_type &&val, link_type left = nullptr, link_type right = nullptr)
    ...
    public:
        ...
        void inorderTraversal(int) {//no stack, Morris Traversal
            link_type par = root;
            link_type left;
            while (par != nil) {
                left = par->left;
    ...
    private:
        link_type minmum(link_type node) {
        ...
        void delete_node(link_type node) {
            ...
            if (root->left == nil)
                ...
                new_root = root->left;
                new_root = splay(node->value, new_root).first;
