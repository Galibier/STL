...
namespace qmj {
    ...
    template<typename Iter>
    struct is_iterator<Iter, void_t<typename iterator_traits<Iter>::iterator_category>> : true_type {
    };
    ...
    struct is_typedef_pod<value_type, void_t<typename value_type::is_POD_tag>> : true_type {
        //		typedef typename value_type::is_POD_tag type;
    };

    template<typename value_type, typename = void>
    struct is_pod : bool_type<type_traits<value_type>::is_POD_type::value ||
                              If<is_typedef_pod<value_type>::value, typename is_typedef_pod<value_type>::type, false_type>::type::value> {
    };
    ...
    template<typename Iter>
    struct is_typedef_mem_copy<Iter, void_t<typename Iter::memory_copy_tag>> : true_type {
        typedef typename Iter::memory_copy_tag type;
    };

    template<typename Iter, typename = void>
    struct is_mem_copy : bool_type<iterator_traits<Iter>::memory_copy_tag::value ||
                                   If<is_typedef_mem_copy<Iter>::value, typename is_typedef_mem_copy<Iter>::type, false_type>::type::value> {
    };
}

#endif //_TYPE_TRAITS_QMJ_
