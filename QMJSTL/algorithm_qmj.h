...
namespace qmj {
    ...
    inline std::pair<Iter1, Iter2> mismatch(Iter1 first1, Iter1 last1, Iter2 first2, const Pred &pred) {
    ...
    inline std::pair<Iter1, Iter2> mismatch(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2) {
    ...
    template<typename IIter1, typename IIter2, typename OIter, typename Comp>
    inline OIter set_intersection(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
    ...
    inline OIter set_intersection(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                                  OIter dest) {
        return (_QMJ set_intersection(first1, last1, first2, last2, dest, std::less<>()));
    }

    template<typename IIter1, typename IIter2, typename OIter, typename Comp>
    inline OIter set_difference(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                                OIter dest, const Comp &cmp) {
        if (first1 != last1 && first2 != last2) {
            for (;;) {
                if (cmp(*first1, *first2)) {
    ...
    inline OIter set_difference(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                                OIter dest) {
        return (_QMJ set_difference(first1, last1, first2, last2, dest, std::less<>()));
    }

    template<typename IIter1, typename IIter2, typename OIter, typename Comp>
    inline OIter set_symmetric_difference(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                                          OIter dest, const Comp &cmp) {
        if (first1 != last1 && first2 != last2) {
            for (;;) {
                if (cmp(*first1, *first2)) {
    ...
    inline OIter set_symmetric_difference(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                                          OIter dest) {
        return (_QMJ set_symmetric_difference(first1, last1, first2, last2, dest, std::less<>()));
    }
    ...
    BIter _partition_imple(BIter first, BIter last, const Pred &pred, std::bidirectional_iterator_tag) {
        for (;; ++first) {
            for (; first != last && pred(*first);)
                ++first;
            if (first == last)
    ...
    inline FIter1 _search_imple(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                                const Pred &pred, std::forward_iterator_tag, std::forward_iterator_tag) {
    ...
    template<typename FIter1, typename FIter2, typename Pred>
    inline FIter1 _search_imple(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                                const Pred &pred, std::random_access_iterator_tag, std::random_access_iterator_tag) {
    ...
    inline FIter1 search(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                         const Pred &pred) {
        return (_QMJ _search_imple(first1, last1, first2, last2,
                                   pred, iter_cate_t<FIter1>(), iter_cate_t<FIter2>()));
    }
    ...
    inline FIter _search_n_imple(FIter first, FIter last, Dif n,
                                 const value_type &val, const Pred &pred, std::forward_iterator_tag) {
        for (; first != last; ++first) {
            if (pred(*first, val)) {
                FIter cur = first;
                ++cur;
    ...
    template<typename FIter, typename Dif, typename value_type, typename Pred>
    inline FIter _search_n_imple(FIter first, FIter last, Dif n,
                                 const value_type &val, const Pred &pred, std::random_access_iterator_tag) {
        if (n <= 0)
            return (first);
        iter_dif_t<FIter> count = last - first;
        FIter result = first;
    ...
    inline FIter search_n(FIter first, FIter last, Dif n, const value_type &val, const Pred &pred) {
        return (_QMJ _search_n_imple(first, last, n, val, pred, iter_cate_t<FIter>()));
    }
    ...
    template<typename FIter1, typename FIter2, typename Pred>
    inline FIter1 _find_end_imple(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                                  const Pred &pred, std::forward_iterator_tag, std::forward_iterator_tag) {
        FIter1 result = last1;
        FIter1 new_result;
        while (true) {
    ...
    inline BIter1 _find_end_imple(BIter1 first1, BIter1 last1, BIter2 first2, BIter2 last2,
                                  const Pred &pred, std::bidirectional_iterator_tag, std::bidirectional_iterator_tag) {
        ...
        RBIter1 rresult = _QMJ search(RBIter1(last1), rlast1, RBIter2(last2), rlast2, pred);
        if (rresult == rlast1)
            return (last1);
        else {
    ...
    inline void replace(FIter first, FIter last,
    ...
    inline OIter
    replace_copy(FIter first, FIter last, OIter dest,
                 const value_type &old_val, const value_type &new_val) {
    ...
    template<typename FIter>
    inline void _rotate_imple(FIter first, FIter middle, FIter last, std::forward_iterator_tag) {
    ...
    template<typename BIter>
    inline void _rotate_imple(BIter first, BIter middle, BIter last, std::bidirectional_iterator_tag) {
    ...
    inline OIter transform(IIter1 first1, IIter1 last1, IIter2 first2, OIter dest, const Fn2 &fn2) {
    ...
    inline bool binary_search(FIter first, FIter last, const value_type &val, const Comp &cmp) {
        FIter result = lower_bound(first, last, val, cmp);
    ...
    template<typename Iter, typename value_type, typename Pred>
    inline iter_dif_t<Iter> _count_fn2(Iter first, Iter last, const value_type &val, const Pred &pred) {
        iter_dif_t<Iter> counter = 0;
        for (; first != last; ++first)
            if (pred(*first, val))
                ++counter;
    ...
    inline bool _match_count(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2, const Pred &pred) {
        _QMJ _match_suffixes(last1, last2, pred, iter_cate_t<Iter1>(), iter_cate_t<Iter2>());
        Iter1 skip;
    ...
    inline bool _is_permutation_imple(RIter1 first1, RIter1 last1, RIter2 first2, RIter2 last2,
                                      const Pred &pred,
    ...
    template<typename FIter1, typename FIter2, typename Pred>
    inline bool _is_permutation_imple(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                                      const Pred &pred, std::forward_iterator_tag, std::forward_iterator_tag) {
    ...
    inline bool is_permutation(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2,
                               const Pred &pred) {
        return (_QMJ _is_permutation_imple(
                first1, last1, first2, last2, pred, iter_cate_t<Iter1>(), iter_cate_t<Iter2>()));
    }
    ...
    template<typename Iter1, typename Iter2, typename Pred>
    inline bool is_permutation(Iter1 first1, Iter1 last1, Iter2 first2, const Pred &pred) {
        for (; first1 != last1; ++first1, ++first2)
            if (!pred(*first1, *first2)) {
                Iter2 last2 = first2;
                _QMJ advance(last2, _QMJ distance(first1, last1));
    ...
    inline void push_heap(RIter first, RIter last, const Comp &cmp) {
        _QMJ _push_heap_imple(first, iter_dif_t<RIter>(last - first - 1), std::move(*(last - 1)), cmp);
    }

    template<typename RIter, typename Dif, typename value_type, typename Comp>
    inline void _heapify(RIter first, Dif index, const Dif len, value_type &&val, const Comp &cmp) {
        Dif right_index = (index << 1) + 2;
        for (; right_index < len;) {
        ...
        if (right_index == len && cmp(val, *(first + (--right_index)))) {
    ...
    inline void _pop_heap_imple(RIter first, RIter last, const Comp &cmp) {
        ...
        *last = std::move(*first);
        _QMJ _heapify(first, iter_dif_t<RIter>(0), iter_dif_t<RIter>(last - first), std::move(val), cmp);
    }
    ...
    inline RIter partial_sort_copy(IIter first, IIter last, RIter dest_first, RIter dest_last,
                                   const Comp &cmp) {
    ...
    inline RIter partial_sort_copy(IIter first, IIter last, RIter dest_first, RIter dest_last) {
        return (_QMJ partial_sort_copy(first, last, dest_first, dest_last, std::less<>()));
    }
    ...
    template<typename RIter, typename Comp>
    inline void _introsort_imple(RIter first, RIter last, size_t depth_limit, const Comp &cmp) {
    ...
    inline std::pair<RIter, RIter> equal_range(RIter first, RIter last, const value_type &val, const Comp &cmp) {
    ...
    inline std::pair<RIter, RIter> equal_range(RIter first, RIter last, const value_type &val) {
    ...
    inline void _merge_adaptive(BIter first, BIter middle, BIter last, Dif len1,
                                Dif len2, pointer buf, Dif buf_size, const Comp &cmp) {
        if (len1 <= len2 && len1 <= buf_size) {
            ...
            BIter new_middle = _QMJ _rotate_adaptive(first_cut, middle, second_cut, len1 - len11, len22, buf, buf_size);
            _QMJ _merge_adaptive(first, first_cut, new_middle, len11, len22, buf, buf_size, cmp);
            _QMJ _merge_adaptive(new_middle, second_cut, last, len1 - len11, len2 - len22, buf, buf_size, cmp);
        }
    }
    ...
    inline void _inplace_merge_imple(BIter first, BIter middle, BIter last, const Comp &cmp) {
        ...
        _QMJ _merge_adaptive(first, middle, last, len1, len2, buf.begin(), iter_dif_t<BIter>(buf.size()), cmp);
    }
    ...
    inline void stable_sort(BIter first, BIter last, const Comp &cmp) {
        ...
        if (1 < len) {
            ...
            _QMJ temporary_buffer<iter_val_t<BIter>> buf((len1 < len2 ? len1 : len2));
            _QMJ _stable_sort_imple(first, last, buf.begin(), iter_dif_t<BIter>(buf.size()), cmp);
        } else
            _insert_sort(first, last, cmp);
    }
